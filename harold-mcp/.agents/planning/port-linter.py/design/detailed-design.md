...
- The `harold_mcp/diagnostics/` package: provider seam, aggregator, the interpreter
  adapter, the heuristic linter (rules, lexical layer, prelude-sort snapshot).
...
- The snapshot maintenance script and the fixtures/tests that pin all of it down.
...
| LP5 | **Provider seam.** A `DiagnosticProvider` `Protocol` (read-only `name` property, `diagnose(source) -> list[ProviderDiagnostic]`) with two initial implementations: `interpreter` (executor-backed, crosses the worker boundary) and `heuristic-linter` (pure text, server process, no `maude` import, no worker dependency). | Q4 |
...
| LP11 | **Prelude snapshot.** A bundled, generated Python module lists every sort the prelude declares, with provenance (Maude version hint, source path, source SHA-256, extraction date). Regeneration is a maintenance script taking the absolute path to a `prelude.maude`; no runtime discovery and no new setting. Only real declarations count (views/renaming mappings/comments/`set` commands/`sorts none .` excluded). | Q10 |
| LP12 | **Deterministic order.** Merged diagnostics are ordered by file position, then by source (`interpreter` first), with whole-file problems last (§5.4). | research §3.6 |
...
src/harold_mcp/
├── diagnostics/
│   ├── __init__.py          # public surface: seam types + the two default providers
│   ├── provider.py          # DiagnosticProvider protocol, seam value types, failures
│   ├── aggregate.py         # collect_diagnostics: run, stamp, order, fail loudly
│   ├── interpreter.py       # InterpreterDiagnosticProvider (executor-backed)
│   ├── heuristic.py         # HeuristicLinterProvider (rule registry runner)
...
│   └── prelude_extract.py   # extractor + module renderer used by the script/tests
├── maude/                   # unchanged (executor, worker); MaudeError docstring amended
└── server/tools/diagnostics.py   # tool + wire models + adapter (rewritten)
scripts/
└── update_prelude_sorts.py  # thin CLI: prelude path → regenerated prelude_sorts.py
tests/
...
graph LR
    ...
    TOOL --> IP["diagnostics/interpreter.py"]
    TOOL --> HP["diagnostics/heuristic.py"]
    ...
    HP --> LEX["lexical.py"]
    RULES --> LEX
    RULES --> DECL["declarations.py"]
    RULES --> SNAP["prelude_sorts.py (generated)"]
    DECL --> LEX
    IP --> MAUDE["harold_mcp.maude (executor client)"]
    SCRIPT["scripts/update_prelude_sorts.py"] --> EXTRACT["prelude_extract.py"]
    EXTRACT --> DECL
    EXTRACT --> SNAP
```

Dependency rule: `diagnostics/` → `maude/` (client side only, for `MaudeExecutor` and
`MaudeError`) and → stdlib. It never imports `fastmcp`, `mcp` or the wire models; the
tool depends on `diagnostics/`, never the reverse.

Package surface (what the tool and tests import from):

```python
# harold_mcp/diagnostics/__init__.py
from harold_mcp.diagnostics.aggregate import DiagnosticCollectionError, collect_diagnostics
from harold_mcp.diagnostics.heuristic import HeuristicLinterProvider
from harold_mcp.diagnostics.interpreter import InterpreterDiagnosticProvider
...
class ProviderFailure:
    """A provider that failed during a collection run."""
    provider: str
    error: Exception
...
```python
class DiagnosticCollectionError(MaudeError):
    ...
    def __init__(self, failures: Sequence[ProviderFailure]) -> None:
        ...
        super().__init__("Diagnostics failed: " + "; ".join(f"{f.provider} ({f.error})" for f in self.failures))
...
1. Run providers **in order**; collect diagnostics per provider; catch `Exception` per
   provider into `ProviderFailure(provider=provider.name, error=exc)` and continue.
...
`DiagnosticCollectionError` subclasses `MaudeError` so every Harold tool error remains
catchable through one base; `harold_mcp/maude/executor.py`'s `MaudeError` docstring is
amended accordingly ("failures of Harold's Maude tooling: interpreter subsystem and
diagnostics providers").

### 4.3 `harold_mcp/diagnostics/interpreter.py` — Maude interpreter provider
...
- Worker crash/timeout errors (`MaudeWorkerCrashedError`/`MaudeWorkerTimeoutError`)
  propagate untouched to the aggregator, which turns them into
  `DiagnosticCollectionError` naming `interpreter`. No pool logic is duplicated.
- `name` is a class attribute, which satisfies the protocol's read-only property.

### 4.4 `harold_mcp/diagnostics/lexical.py` — the code view
...
### 4.5 `harold_mcp/diagnostics/declarations.py` — sort declaration reader
...
`../research/probes/extract_prelude_sorts.py`; extraction now yields 165 names from 25
modules, no junk):
...
4. Excluded: names in **view bodies** (`view … is … endv`), any statement containing
   ...
   `none`.
5. Module names are the plain first token (`LIST*{X :: TRIV}` → `LIST*`), used only for
   provenance/messages.

### 4.6 `harold_mcp/diagnostics/rules.py` — the rules
...
| 7 | `prelude-sort-redeclared` | info | each `SortDeclaration` whose name is in `PRELUDE_SORTS`. Span = the declared name | — | `redeclare_prelude.maude` (existing) |
...
### 4.7 `harold_mcp/diagnostics/heuristic.py` — the heuristic provider
...
internal→wire adapter. The flow:
...
def _to_range(diagnostic: ProviderDiagnostic) -> MaudeRange | None:
    ...
    start = MaudePosition(line=diagnostic.line, column=diagnostic.column)
        ...
        MaudePosition(line=diagnostic.line, column=diagnostic.end_column)
...
### 4.9 `harold_mcp/diagnostics/prelude_sorts.py` + `scripts/update_prelude_sorts.py`

Generated module (never hand-edited), written by the script:

```python
# Generated by scripts/update_prelude_sorts.py — do not edit by hand.
# Maude version: 3.5.1
...
# Content: 165 sort names declared by 25 modules.
...
- `prelude_extract.py` holds the logic (`extract_prelude_sorts(lines) -> dict[str, str]`,
  `render_snapshot_module(sorts, *, source_path, source_hash, maude_version, extracted)`
  → `str`) so it is importable by tests and type-checked by mypy; the reader is
  `declarations.iter_sort_declarations` (§4.5) — the same code path rule 7 uses on user
  code, which is why the snapshot and the lint rule can never disagree on what a
  declaration is.

`scripts/update_prelude_sorts.py` (thin CLI, no logic):

```console
$ uv run python scripts/update_prelude_sorts.py /path/to/prelude.maude
Extracted 165 sort names from 25 modules
Wrote src/harold_mcp/diagnostics/prelude_sorts.py
$ uv run python scripts/update_prelude_sorts.py /path/to/prelude.maude --check
...
`prelude_sorts.py`), `--check`, `--maude-version` (default: inferred from the path when
it contains `Maude-<version>`, else `unknown`). `--check` compares the **data** — it
imports the committed module and compares `PRELUDE_SORTS`/`PRELUDE_SORT_BASES` with a
...
comment, not the data). Guard rails: the script refuses to write a snapshot with fewer
than 50 names (the extractor silently breaking is the realistic failure mode), and prints
...
| `src/harold_mcp/maude/executor.py` | `MaudeError` docstring broadened (now also the base of `DiagnosticCollectionError`) |
| `docs/modules.md` | Add `::: harold_mcp.diagnostics` entries (provider, aggregate, interpreter, heuristic, rules, lexical, declarations, prelude_sorts, prelude_extract) |
| `README.md` | "Harold MCP Tools" entry: both sources, severities, `info`, fixes, ordering |
| `DEVELOPER_GUIDE.md` | New "Maintaining the prelude sort snapshot" section: when to regenerate (Maude upgrade), the command, `--check` |
...
| `pyproject.toml` | Suggested minor bump `0.0.4.dev0` → `0.0.5.dev0` |
| `.agents/summary/`, `AGENTS.md` | Prompt the user to re-run the codebase-summary skill (new package, new modules) |
| `improve-rag/improvement/linter.py` | Untouched |
...
| Situation | Behavior |
| --- | --- |
| Path missing/unreadable | `MaudeFileNotFoundError` before any provider runs (unchanged, v1-R5) |
| Unrecoverable parse error in the file | diagnostic (`interpreter`/`compiler`/`error`, `range=null`), not a tool error |
| Worker crash / timeout during the interpreter provider | `DiagnosticCollectionError` (from `MaudeWorkerCrashedError`/`TimeoutError`), message names `interpreter` and the cause; no partial results; next call recovers on the recreated pool (v1 behavior preserved) |
| Unexpected exception in a rule | same aggregate failure path, naming `heuristic-linter` — a rule bug must not silently degrade the tool |
...
| File | Covers |
| --- | --- |
| `test_diagnostics_lexical.py` | `code_view` masking per region (comments `***`/`---`, string literals incl. `***`/`--` inside, quoted identifiers incl. `'--`/`'when`, labels), length preservation, column fidelity; `declaration_index` (vars, inline `X:Sort`, ops, sort references); `SourceView.from_text`/`sort_bases` |
| `test_diagnostics_declarations.py` | `iter_sort_declarations`: multi-line statements, parameterized module headers, `view … is endv` on one line, view bodies and `sort X to Y` mappings excluded, `sorts none .` and the `endsth)` meta-term junk excluded, `?`-suffixed names accepted, `Name`/`line`/`column`/`module` values |
| `test_diagnostics_rules.py` | one positive test per rule (code, severity, line, exact column span, message substring, fix presence/edits), and the hardening negatives: `when`/`--`/`=`/non-ASCII inside strings and quoted identifiers, `op when`/`op _--_`/`var when` declarations, capitalized labels, inline `X:List{Nat}` sort references, `True` silenced by the keyword list (documented behavior, Appendix D.3), and the documented `when`-operator-usage false positive (Appendix D.7) |
| `test_diagnostics_providers.py` | `InterpreterDiagnosticProvider` (warning mapping, synthesized error, `source`/`code` stamping, timeout/crash propagation) and `HeuristicLinterProvider` (stamping from the registry, custom rule subset) |
| `test_diagnostics_aggregate.py` | ordering (file order, provider tie-break, whole-file last), no-failure merged provenance, one and two failing providers → `DiagnosticCollectionError` naming each with its cause, **no partial results**, chained cause, unexpected `ValueError` from a rule |
...
| `test_diagnostics_prelude.py` | snapshot invariants (`Qid`→`QID`, `Nat`, `Elt`, `List{X}`, `State`; no `none`, no `NatList`, no name with whitespace/`.`; > 100 names; bases ⊂ names modulo parameters) and `extract_prelude_sorts` against an inline sample prelude covering every exclusion rule |
| `test_update_prelude_sorts_script.py` | the CLI end-to-end on a temp fixture with `--output <tmp path>`: writes a loadable module, `--check` passes on the result and fails on a modified snapshot, refuses a junk extraction (< 50 names) |
...
`scripts/update_prelude_sorts.py --check` is the reproducible verification for LP11; it is
...
is byte-identical to a fresh extraction of that prelude. The committed snapshot is
asserted by invariants (not byte equality with the installation), because CI does not ship
a Maude installation file.
...
| Rule 1 granularity | one diagnostic per offending character | one per line (source linter) | Spans and single-char fixes are unambiguous; "apply every fix" converges in one pass (D1) |
...
| Result order | file order, interpreter first on a line | provider-grouped; interpreter-first everywhere | File order matches how agents fix code; Maude's missing columns make column-only interleaving impossible |
...
6. **The prelude extraction is executable and junk-free**: 165 names from 25 modules with
   the §4.5 rules (view bodies: 99 names excluded, renamings: 52, `none`: 8, meta-term
   junk: 2), including `List{X}`/`Set{X}`/`Map{X,Y}`/`$Split{X}`, `Elt`, and the
   META/LOOP-MODE/CONFIGURATION sorts.
7. **MCP cannot express partial results** (`integration-options.md` §3.7 and Q3's
   verification): errors are `isError`-only, so provider failures must fail the call.
...
| Runtime read of the installed `prelude.maude` (Q10 option b) | Rejected (Q10) | Path discovery/configuration, unreadable-file modes, and parser work for a maintenance-time concern; the `--check` script covers drift |
...
### Appendix E — Decisions to confirm during design review

D1, D3 and D7 contradict the *literal* wording of confirmed answers (Q2, Q5, Q9). They are
proposed here because the port's evidence points the other way; they need an explicit
confirmation before implementation (or the design is amended back to the letter of the
answers).

1. **D1 (positions)** — *deviates from Q2*: heuristic diagnostics populate
   `range.start.column` **and** `range.end` for *all* findings, not only fix-carrying ones
   (Q2 anticipated columns for fix-carrying diagnostics; a span per finding is more useful
   and uniform, and every rule already knows the token it matched).
2. **D2 (rule 1 granularity)**: one diagnostic per offending non-ASCII character
   (Q2's fix payload assumes single-character edits) instead of one per line as in the
   source linter.
3. **D3 (rule 1 code)** — *deviates from Q5*: `non-ascii-character` instead of the working
   name `unicode-punctuation` (§5.2), because the rule fires on any non-ASCII character
   (`café` has no punctuation to replace).
4. **D4 (ordering)**: file order with `interpreter` first on a line, whole-file problems
   last (§5.4) — v1 emitted warnings first and the synthesized error last, which this
   keeps.
5. **D5 (`code` typing)**: `code: str` (open) vs `Literal` (closed) — Appendix A.
6. **D6 (prelude scope)**: the snapshot includes theory sorts (`Elt`) and the
   meta-level/object sorts, per the letter of Q7; `Elt` will therefore produce `info`
   findings in parameterized user modules.
7. **D7 (rule 6 keyword list)** — *deviates from Q9(b)*, which asked to revisit the
   allow-list: `MAUDE_KEYWORDS` is kept verbatim from the source linter, so `True`/`False`
   remain silenced (Appendix D.3); the hardening spent on rule 6 is the sort-aware
   allow-list (`sort_bases`, `sort_references`, `PRELUDE_SORT_BASES`) instead. Flipping to
   exact (case-sensitive) keyword matching is a one-line change plus a fixture if the user
   prefers it.
